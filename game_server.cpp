...
int main(int argc, char *argv[])
{
  ...
  try
  {
    ...
    int randomID = 0, randomXY = 0, count = 0, timer = 1000;
    bool flag = true;
    ...
    while (true)
    {
      ...
      if (randomXY == 0)
      {
        if (client_list[randomID].x() > client_list[0].x() && client_list[0].x() < 50)
      ...
      {
        if (client_list[randomID].y() > client_list[0].y() && client_list[0].y() < 50)
